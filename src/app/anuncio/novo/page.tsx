...
export default function NovaPaginaAnuncio() {
  ...
  // botão: geolocalização explícita
  ...
  // quando coords mudar, resolve cidade (bem leve; 1 request)
  useEffect(() => {
    let stop = false;
    (async () => {
      if (!coords) return;
  ...
  // fallback por CEP → precisa só aproximar (usamos Nominatim também)
  const locateByCEP = async () => {
    const digits = (cep || "").replace(/\D/g, "");
    if (digits.length < 8) return alert("Informe um CEP válido (8 dígitos).");
    try {
      const url = `https://nominatim.openstreetmap.org/search?format=jsonv2&countrycodes=br&postalcode=${digits}&limit=1`;
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="container mx-auto max-w-6xl px-4 py-8">
        ...
        {/* Grid principal: formulário + preview */}
        ...
        {/* MAPA: em uma seção ABAIXO de tudo */}
        <section className="mt-8 rounded-2xl border border-white/10 bg-card p-5">
          ...
          <p className="mt-2 text-xs text-zinc-500">
            Dica: se a localização não aparecer automaticamente, clique em
            “Usar minha localização” acima. Caso negue o acesso, informe seu CEP.
          </p>
        </section>
      </div>
    </main>
