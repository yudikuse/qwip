...
export default function ImageEditorModal({ file, open, onClose, onApply }: Props) {
  ...
  // Carrega a imagem no canvas quando abrir o modal ou trocar o arquivo
  useEffect(() => {
    if (!open || !file) return;
    (async () => {
    ...
    try {
      ...
      // avisa botão externo que começou
      ...
      const blob = await aiRemoveBackground(file, (current, total) => {
        ...
        // emite para o botão compacto no preview
  ...
  function onAdjust() {
    if (!canvasRef.current) return;
    // redesenha a base atual (canvas em si) e aplica brilho/contraste
    drawImageToCanvas(canvasRef.current, canvasRef.current);
  ...
  // Recorte simples: arraste para selecionar
  useEffect(() => {
    if (!open || !canvasRef.current || !overlayRef.current) return;
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/60">
      <div className="w-[min(100%,1000px)] max-h-[90vh] rounded-2xl bg-zinc-900 shadow-2xl border border-zinc-800 overflow-hidden">
        ...
        <div className="grid md:grid-cols-[1.2fr,0.8fr] gap-4 p-4">
          ...
          <div className="space-y-4">
            ...
            <div className="p-3 rounded-xl bg-zinc-950 border border-zinc-800">
              ...
              <p className="text-xs text-zinc-500 mt-2">
                Arraste na imagem para selecionar a área.
              </p>
            </div>
        ...
        <div className="px-4 pb-4 text-xs text-zinc-500">
          Dica: na primeira vez pode demorar um pouco para baixar o modelo. Depois,
          fica mais rápido no mesmo navegador.
        </div>
      </div>
    </div>
  );
